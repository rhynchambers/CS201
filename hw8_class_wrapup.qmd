...
#### You use a violin plot when you want to visualize the distribution of a continuous variable across different categories that will show the density which is where the most common one is.

```{r}
ggplot(penguins_data, aes(x = species, y = flipper_length_mm)) + 
  geom_violin()
...
#### A box plot will show you the central tendency(median), the spread(IQR), the outliers, skewness, and the range.

```{r}
ggplot(penguins_data, aes(x = flipper_length_mm, y = species)) +
  geom_boxplot()
```
